...
export default function Card({eventid, e}) {
    if(e){
...
return(
    <div className="embla__slide p-5 w-[400px]" >
      ...
      >
        <img src={event.posters[0]} className="object-fill w-full h-full"/>
        <Link to={'/workshop/'+ event.id} className="absolute bottom-10 left-28 border border-yellow-300 w-36 rounded-lg text-center drop-shadow-lg bg-yellow-300 font-primary text-xl px-2 py-1">Register</Link>
      </div>
    </div>
  )
  }
